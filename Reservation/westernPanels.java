...
public class westernPanels{
	...
	private String[] bye = new String[5];
	private String[] hi = new String[5];
	...
	private Java2sAutoComboBox destinations;
	...
	List<String> availDestinations = new ArrayList<String>();
	...
	public void addItemListenerHelperComboBennie() {
		destinations.addItemListener(destinationListener = new ItemListener() {
            ...
            try {
            	...
            	List<String> availDestinations = DestinationList.destList(ports);
            	double dist = DestinationList.distance(availDestinations.get(pickedOperations), ports);
            	bye = DestinationList.getAGoTime();
            	hi = DestinationList.getAByeTime(bye, dist);
            	departures = new JComboBox(bye);
            	arrivals = new JComboBox(hi);
	...
	public void grabMetheOperations() {
		try {
			...
			availDestinations = DestinationList.destList(ports);
			//double dist = DestinationList.distance(availDestations.get(pickedOperations, ports);
			destinations = new Java2sAutoComboBox(availDestinations);
		...
		destinations.removeItemListener(destinationListener);
			destinations.addItemListener(destinationListener = new ItemListener() {
	            ...
	            try {
	            	...
	            	westernPanel.revalidate();
	            	westernPanel.repaint();
	            	departures.setModel(new DefaultComboBoxModel(bye));
	            	arrivals.setModel(new DefaultComboBoxModel(hi));
	            	departures.setSelectedIndex(-1);
	            	arrivals.setSelectedIndex(-1);
	            	westernPanel.revalidate();
	            	westernPanel.repaint();
